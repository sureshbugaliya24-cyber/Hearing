...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(empty($title) || empty($content)) {
        ...
        $stmt = $conn->prepare("INSERT INTO blog_posts (title, excerpt, content, image_url) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("ssss", $title, $excerpt, $content, $image_url);
        
        if ($stmt->execute()) {
            header("Location: dashboard.php?msg=Post added successfully");
            exit;
        } else {
...
<body class="bg-gray-50 flex h-screen overflow-hidden">
    ...
    <div class="w-64 bg-[#111827] text-white flex flex-col hidden md:flex">
        ...
        <nav class="flex-grow p-4">
            ...
            <a href="add_blog.php" class="block py-3 px-4 mt-2 rounded transition duration-200 bg-[#00a8d6] text-white font-semibold">
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
        <header class="bg-white shadow flex justify-between items-center p-4">
            <h2 class="text-xl font-bold text-gray-800">Add New Post</h2>
            <a href="dashboard.php" class="text-gray-600 hover:text-gray-900"><i class="fas fa-arrow-left mr-1"></i> Back</a>
        </header>
            ...
            <div class="bg-white rounded-lg shadow-md p-8">
                <form action="" method="POST">
                    <div class="mb-6">
                        ...
                        <input type="text" name="title" required class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00a8d6]" value="<?php echo isset($_POST['title']) ? htmlspecialchars($_POST['title']) : ''; ?>">
                    </div>

                    <div class="mb-6">
                        <label class="block text-gray-700 font-bold mb-2">Image URL (Featured Media)</label>
                        <input type="url" name="image_url" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00a8d6]" placeholder="https://example.com/image.jpg" value="<?php echo isset($_POST['image_url']) ? htmlspecialchars($_POST['image_url']) : ''; ?>">
                    </div>

                    <div class="mb-6">
                        <label class="block text-gray-700 font-bold mb-2">Excerpt (Short Description)</label>
                        <textarea name="excerpt" rows="3" class="w-full px-4 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#00a8d6]"><?php echo isset($_POST['excerpt']) ? htmlspecialchars($_POST['excerpt']) : ''; ?></textarea>
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <textarea id="hidden-content" name="content" class="hidden"><?php echo isset($_POST['content']) ? htmlspecialchars($_POST['content']) : ''; ?></textarea>
                    </div>
